...
int injection_init(struct precision * ppr,
  ...
  //llh read the axion file and fix the value of f at f(z=300) independently of z and x
  else if(pin->chi_type == chi_from_axion_file){
    class_call(injection_read_f300_from_axionfile(ppr, pth,pin),
  	       pin->error_message,
  	       pin->error_message);
  }
...
int injection_deposition_function_at_z(struct injection* pin,
  ...
  if(z > pin->z_start_chi_approx){
    ...
    //llh get f300 from axion file
    else if(pin->chi_type == chi_from_axion_file){
      
      pin->chi[pin->index_dep_heat]  = pin->fheat300;
      pin->chi[pin->index_dep_ionH]  = pin->fHion300; 
      pin->chi[pin->index_dep_ionHe] = pin->fHeion300;
      pin->chi[pin->index_dep_lya]   =  pin->flya300;
      pin->chi[pin->index_dep_lowE]  =  pin->fcont300;
      
      
      /* printf("%e %e %e %e %e\n",pin->chi[pin->index_dep_heat],pin->chi[pin->index_dep_ionH],pin->chi[pin->index_dep_ionHe],pin->chi[pin->index_dep_lya],pin->chi[pin->index_dep_lowE]); */
    }
...
int injection_rate_DM_decay(struct injection * pin,
  ...
  /** - Calculate injection rates */
  /**llh brutaly added a feff=0.1 factor here**/
  /* *energy_rate = 0.1*pin->rho_cdm*pin->DM_decay_fraction*pin->DM_decay_Gamma* */
...
/**
 *llh: Infer the chi_alpha(z=300) and make a table for class to be happy. 
chi_z300_axion is defined in include/precision.h
*/


int injection_read_f300_from_axionfile(struct precision * ppr,
				       struct thermodynamics * pth,
				       struct injection* pin
				       ) {
  ...
  int num_f= 5;//number of different efficiencies
  int num_ga=0;
  int num_ma=0;
  ...
  double * logma=NULL;
  double * logf=NULL;
  double * logf_temp=NULL;
  double * logf_at_logga=NULL;
  double * logfvalue=NULL;
  ...
  /* the following file is assumed to contain (apart from comments and blank lines):
     ...
     - 5 columns (ma, ga,  fh300_,fexc300,fHion300,fHeion300,fcont300 ) where  fh300_,fexc300,fHion300,fHeion300,fcont300  are computed at effeciencies for energy deposition from DM decays at z=300.
     - ma and ga are assumed to be arranged as:
    logma1 logga1  fh300_,fexc300,fHion300,fHeion300,fcont300 
    ..
    logmaN logga1  fh300_,fexc300,fHion300,fHeion300,fcont300 
    logma1 logma2  fh300_,fexc300,fHion300,fHeion300,fcont300 
    ..
    logmaN logga2  fh300_,fexc300,fHion300,fHeion300,fcont300 
    etc
  */
  ...
  class_open(fA,ppr->chi_z300_axion, "r",pin->error_message);
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
	...
	//printf("%i %i\n",num_ma,num_ga);
        ...
        class_alloc(logma,num_ma*sizeof(double),pin->error_message);
        class_alloc(logga,num_ga*sizeof(double),pin->error_message);
	class_alloc(logfvalue,num_f*sizeof(double),pin->error_message);
	//final vector with 5 values
	
	class_alloc(logf,num_f*num_ma*num_ga*sizeof(double),pin->error_message);
	class_alloc(logf_temp,num_ma*num_ga*sizeof(double),pin->error_message);
	class_alloc(logf_at_logga,num_ma*sizeof(double),pin->error_message);
	...
	array_line=0;
        ...
        /* read (logma,logga1,fh300,fexc300,fHion300,fHeion300,fcont300 ) */
      
	class_test(sscanf(line,"%lg %lg %lg %lg %lg %lg %lg",&(logma[array_line%num_ma]),//llh:% means module
			  &(logga[array_line/num_ma]),
			  &(logf[array_line]),&(logf[num_ma*num_ga+array_line]),&(logf[2*num_ma*num_ga+array_line]),
			  &(logf[3*num_ma*num_ga+array_line]),&(logf[4*num_ma*num_ga+array_line])) != 7,
		   pin->error_message,
                   "could not read value of parameters (ma,ga,f) in file %s\n",ppr->chi_z300_axion);
        array_line ++;
      }
    }
  }
  ...
  fclose(fA);
  //loop over the selected num_f efficiencies
  for(ieff=0; ieff<num_f; ++ieff){
    //extract each of the logf
    for(itemp=0; itemp<num_ma*num_ga; ++itemp){
      logf_temp[itemp]=logf[ieff*num_ma*num_ga+itemp];
    }
    //printf("%f \n",logf_temp[2],stderr);
    ...
    class_call(array_interpolate_linear(logga,
					...
					logf_at_logga,
					num_ma,
					pin->error_message), pin->error_message,
	       pin->error_message);

    /* for(itemp=0; itemp<num_ma; ++itemp){ */
    /*   printf("%f logf[%i]=%f \n",logma[itemp],ieff,logf_at_logga[itemp]); */
    /* } */
    //
  ...
  //xevec=logxevalue;
  //values of f300 to be feeded to chi in 
  pin->fheat300= pow(10.0,logfvalue[0]);
  pin->fHion300=pow(10.0,logfvalue[2]);
  pin->fHeion300=pow(10.0,logfvalue[3]);
  pin->flya300=pow(10.0,logfvalue[1]);
  pin->fcont300= pow(10.0,logfvalue[4]);

  
 
    /* printf("after interpolation of chi for ma= %f and ga= %e, fh300_,fexc300,fHion300,fHeion300,fcont300 =\n",pth->mDMdec,pth->gDMdec); */
    /* for(ieff=0; ieff<num_f; ++ieff){ */
    /*   printf("%e, ", pow(10.0,logfvalue[ieff])); */
    /* }; */
    /* printf("\n"); */
