...
int thermodynamics_set_parameters_reionization(
  ...
  case reio_inter:
    ...
    //llh    do this to use the interpolation! if you comment below, the reio_inter parameters provided in the ini file will be considered
    class_call(
	       thermodynamics_reioz_fg(ppr,pth),
               pth->error_message,
               pth->error_message);
    //printf("test 2 Pointer to value: %f\n", *test);
...
int thermodynamics_derivs(
  ...
  **/
   ...
   */
  //llh
  //printf("reionization_z_start_max= %e\n", ppr->reionization_z_start_max);
...
int thermodynamics_sources(
  ...
  //llh a n_e sigma_T
  //printf("%e %e",z,(1.+z) * (1.+z) * ptw->SIunit_nH0 *  sigmaTrescale * _sigma_ * _Mpc_over_m_);
  return _SUCCESS_;
}
...
int thermodynamics_reionization_get_tau(
  ...
  for (index_z=0; index_z<pth->tt_size-1; index_z++) {
    if (pth->thermodynamics_table[index_z*pth->th_size+pth->index_th_xe] < x_e_min) {
      ...
      //llh looking for the index of the min in xe
      //printf("index_z= %e",index_z )
    }
  }
  //llh looking for the index of the min in xe
  //printf("index_reio_start= %i, zmin= %e, xemin= %e\n",index_reio_start, pth->z_table[index_reio_start],x_e_min);
...
int thermodynamics_ionization_fractions(
  ...
  /** - --> seventh regime: calculate x_noreio during reionization
    ...
    //llh: when large energy injection Hyrec can give rise to x>>1 :S
    if (pth->has_exotic_injection == _TRUE_&& x>1 ) {
      x=1;
      ptdw->x_H = x_H/(x_H + ptw->fHe * x_He);
      ptdw->x_He = x_He/(x_H + ptw->fHe * x_He);
...
/**
 *llh: Infer the xe(z) at z= 6.01, 6.311, 6.625, 6.952, 7.293, 7.648, 8.019, 8.406, 8.81, 9.23, 9.669, 10.13, 10.6, 11.1, 11.62, 12.16, 12.73, 13.32, 13.93, 14.57, 15.09(where Puchwein reionization is hapening) from file to feed it to reio_inter. Inspired by  thermodynamics_helium_from_bbn from thermodynamics.c
*/
int thermodynamics_reioz_fg(struct precision * ppr,
	     struct thermodynamics * pth
			    ) {
  ...
  //change these numbers of redshifts  according to your data file
  int num_z=21;
  int num_ga=0;
  int num_ma=0;
  //the number of entries of the reion array for class is sligthly larger with 3 entries at z=0,3 and 4 and a last one at z=zend+epsilon with xe values=-2.0, -2.0, -1.0 and 0.0 by default in FG reionisation.
  int num_vec=num_z+4;
  //change these values of reio_inter_z according to your reio:
  double reio_vec_z[25]={0, 3, 4, 6.01, 6.311, 6.625, 6.952, 7.293, 7.648, 8.019, 8.406, 8.81, 9.23, 9.669, 10.13, 10.6, 11.1, 11.62, 12.16, 12.73, 13.32, 13.93, 14.57, 15.09, 15.0915};
  //change these values of reio_inter_xe according to your reio, here default value are FG, with the 3 first entries and the last entries default:
  double reio_vec_xe[25]={-2, -2, -1, 1.07843, 1.01999, 0.892031, 0.769839, 0.662217, 0.567022, 0.48317, 0.409166, 0.343875, 0.286748, 0.237083, 0.193999, 0.157, 0.124968, 0.0972747, 0.0737904, 0.0537032, 0.0363078, 0.0214783, 0.00887156, 0.000335738, 0};
 
  
  //input values arrising from the thermodynamics input
  double logmavalue;
  double loggavalue;
  ...
  double * logma=NULL;
  double * logxe=NULL;
  double * logxe_temp=NULL;
  double * logxe_at_logga=NULL;
  double * logxevalue=NULL;
  ...
  /* the following file is assumed to contain (apart from comments and blank lines):
     - the two numbers (num_ma, num_ga) = number of values of values of ga and ma run in DH
     - 23 columns (ma, ga, xe1,...,xe21) where log xe1 ...logxe21 are computed at redshifts z1...z21 within reionization period.
     - ma and ga are assumed to be arranged as:
    logma1 logga1 logxe
    ..
    logmaN logga1 logxe
    logma1 logga2 logxe
    ...
    etc
  */
  //ppr is the pointer to precision structure and we have define xeaxion_file in include/precision.h
  /* llh: this stops class if mDM dec or gDM dec are not provided when selecting reio_inter obviously, we would need to define a new reio_inter in the case of injection, se reio_inter_DH in progress */
  class_test(pth->mDMdec<=0. || pth->gDMdec<=0.,
	     pth->error_message,
	     "mDMdec<=0 or gDMdec<=0, no FG reio will be taken into account");
   //this is just to check if inputs above are self consitent 
  class_test(sizeof(reio_vec_xe) / sizeof(reio_vec_xe[0])!=num_vec||sizeof(reio_vec_z) / sizeof(reio_vec_z[0])!=num_vec||pth->reio_inter_num!=num_vec,
	     pth->error_message,
	     "size of x and z arrays inconsistent in reio_inter");

  logmavalue=log10(pth->mDMdec);
  loggavalue=log10(pth->gDMdec);
  //pth->reio_inter_num=num_vec;
  //printf("%lf %lf\n",logmavalue,loggavalue);
  
  //this is to prevent class to run in a region where monthepython breaks down                                                                                                
  //due to the limits of the Planck likelihood ok region
  //FOR TANH reio this is NOT USED in thermodynamics.c BUT in heating.c, yet I updated the fit to the not oK zone                                                                                    
  class_test(loggavalue> -9.67808 - 1.85551 *logmavalue + 1.02277* log(logmavalue),
             pth->error_message,
             "the combination of mDMdec and gDMdec is beyond the range of parameters that can be handled by Planck likelyhoods (too much energy injection)");
  
  class_open(fA,ppr->xeaxion_file, "r",pth->error_message);
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      ...
      /* if the line contains data, we must interpret it. If (num_omegab, num_deltaN)=(0,0), the current line must contain
         their values. Otherwise, it must contain (logma, logga, xe1,...,xe10). */
      if ((num_ma==0) && (num_ga==0)) {

        /* read (num_ma, num_ga), infer size of arrays and allocate them */
        class_test(sscanf(line,"%d %d",&num_ma,&num_ga) != 2,
                   pth->error_message,
                   "could not read value of parameters (num_ma,num_ga) in file %s\n",ppr->xeaxion_file);
	//printf("%i %i\n",num_ma,num_ga,stderr);
	
        class_alloc(logma,num_ma*sizeof(double),pth->error_message);
        class_alloc(logga,num_ga*sizeof(double),pth->error_message);
	//final logxevalue of length num_z
	class_alloc(logxevalue,num_z*sizeof(double),pth->error_message);
	//intermediate vectors of size num_ma*num_ga and num_ma to interpolate in order to get each logxe[i], i=1.. num_z
	//class_alloc(logxe,num_ma*num_ga*sizeof(double),pth->error_message);
	class_alloc(logxe,num_z*num_ma*num_ga*sizeof(double),pth->error_message);
	class_alloc(logxe_temp,num_ma*num_ga*sizeof(double),pth->error_message);
	class_alloc(logxe_at_logga,num_ma*sizeof(double),pth->error_message);
	...
	class_test(sscanf(line,"%lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg",&(logma[array_line%num_ma]),//llh:% means module
			  &(logga[array_line/num_ma]),
			  &(logxe[array_line]),&(logxe[num_ma*num_ga+array_line]),&(logxe[2*num_ma*num_ga+array_line]),
			  &(logxe[3*num_ma*num_ga+array_line]),&(logxe[4*num_ma*num_ga+array_line]),&(logxe[5*num_ma*num_ga+array_line]),
			  &(logxe[6*num_ma*num_ga+array_line]),&(logxe[7*num_ma*num_ga+array_line]),&(logxe[8*num_ma*num_ga+array_line]),
			  &(logxe[9*num_ma*num_ga+array_line]),&(logxe[10*num_ma*num_ga+array_line]),
			  &(logxe[11*num_ma*num_ga+array_line]),&(logxe[12*num_ma*num_ga+array_line]),
			  &(logxe[13*num_ma*num_ga+array_line]),&(logxe[14*num_ma*num_ga+array_line]),&(logxe[15*num_ma*num_ga+array_line]),
			  &(logxe[16*num_ma*num_ga+array_line]),&(logxe[17*num_ma*num_ga+array_line]),&(logxe[18*num_ma*num_ga+array_line]),
			  &(logxe[19*num_ma*num_ga+array_line]),&(logxe[20*num_ma*num_ga+array_line])
                          ) != 23,
		   pth->error_message,
                   "could not read value of parameters (ma,ga,xe) in file %s\n",ppr->xeaxion_file);
        array_line ++;
      }
    }
  }
  //printf("%lf %lf\n",logga[4],*logma,stderr);
  fclose(fA);
  //loop over the selected num_z redshifts for reionisation(see file)
  for(ixeion=0; ixeion<num_z; ++ixeion){
    //extract the logxe at the right z
    for(itemp=0; itemp<num_ma*num_ga; ++itemp){
      logxe_temp[itemp]=logxe[ixeion*num_ma*num_ga+itemp];
    }
    //printf("%f \n",logxe_temp[2],stderr);
    /** - interpolate in one dimension along logga to get logxe[logma] at  loggavalue*/
    class_call(array_interpolate_linear(logga,
					num_ga,
					logxe_temp,
					num_ma,
					loggavalue,
					...
					logxe_at_logga,
					num_ma,
					pth->error_message), pth->error_message,
	       pth->error_message);
    //intermediate printing to understand what's going on
    /*  for(itemp=0; itemp<num_ma; ++itemp){ */
    /*   printf("%f logxe[%i]=%f \n",logma[itemp],ixeion,logxe_at_logga[itemp]); */
    /* } */
   
    /** - interpolate in remaining dimension along logma  to get logxe_at_logga at logmavalue */
    class_call(array_interpolate_linear(logma,
					num_ma,
					logxe_at_logga,
					1,
					logmavalue,
  ...
  for(ixeion=0; ixeion<num_vec; ++ixeion){
    pth->reio_inter_z[ixeion]=reio_vec_z[ixeion];
    //printf("Before re-assign [default   data]: %f %f\n", pth->reio_inter_z[ixeion],pth->reio_inter_xe[ixeion]);
    if(ixeion>=3 && ixeion<num_vec-1){
      pth->reio_inter_xe[ixeion]=pow(10.0,logxevalue[ixeion-3]);
    }
    else{
      pth->reio_inter_xe[ixeion]=reio_vec_xe[ixeion];
    }
    //printf("after reasign [interpolated data]: %f %f\n", pth->reio_inter_z[ixeion],pth->reio_inter_xe[ixeion]);
  }
  //printf("Pointer to value: %f\n", *logxevalue);
