...
int input_read_parameters_general(struct file_content * pfc,
  ...
  /*llh*/
  /* Read axion mass and coupling I have to read these parameters before choosing reio if not monthepython does not find it*/
  class_read_double("mDMdec",pth->mDMdec);
  class_read_double("gDMdec",pth->gDMdec);
  ...
  if (flag1 == _TRUE_){
    ...
    //llh
    else if (strcmp(string1,"reio_inter_DH") == 0){
      pth->reio_parametrization = reio_inter_DH;
    }
    else{
      class_stop(errmsg,
                 "You specified 'reio_parametrization' as '%s'. It has to be one of {'reio_none','reio_camb','reio_bins_tanh','reio_half_tanh','reio_many_tanh','reio_inter','reio_inter_DH'}.",string1);
    }
  }
  ...
  case reio_inter:
    ...
    /** 8.e) reionization parameters if reio_parametrization=reio_inter_DH */
  case reio_inter_DH:
    /* Read */
    class_read_int("reio_inter_num",pth->reio_inter_num);
    class_read_list_of_doubles("reio_inter_z",pth->reio_inter_z,pth->reio_inter_num);
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (flag1 == _FALSE_) {
    /* Fill with Lambda */
    /*llh added Omega h^2cdm*/
    pba->Omega0_lambda= 1. - pba->Omega0_k - Omega_tot;
    if (input_verbose > 0){
      printf(" -> matched budget equations by adjusting Omega_Lambda = %g for Omega_cdm h^2= %g, h=%g, Omegab=%g \n",pba->Omega0_lambda,pba->h*pba->h*pba->Omega0_cdm,pba->h, pba->Omega0_b);
    }
  }
...
int input_read_parameters_injection(struct file_content * pfc,
  ...
  //llh for axion decay
  double GeVm1tos = 6.582e-25;//units
  ...
  /** llh: axion Decay width in 1/s for gDMdec in 1/GeV and mDMdec in eV*/
  if(pth->gDMdec>0 && pth->mDMdec>0){
    pin->DM_decay_Gamma=pow(pth->gDMdec,2.0)*pow(pth->mDMdec*1.e-9,3.0)/64./_PI_/GeVm1tos ;//in 1/s
    /* Test */
    /*  printf("g= %e /GeV, m= %e eV, Gamma = %e /s\n",pth->gDMdec ,pth->mDMdec,pin->DM_decay_Gamma ); */
  } 
  ...
  if (flag1 == _TRUE_){
    ...
    //llh
    else if (strcmp(string1,"from_axion_file") == 0){
      pin->chi_type = chi_from_axion_file;
    }
     else{
      class_stop(errmsg,
                   "You specified 'chi_type' as '%s'. It has to be one of {'CK_2004','PF_2005','Galli_2013_file','Galli_2013_analytic','heat','from_x_file','from_z_file','from_axion_file'}.",string1);
    }
  }
  ...
  if(pin->chi_type == chi_from_x_file || pin->chi_type == chi_from_z_file){
    /** 6.a) External file */
    /* Read */
    /*llh, I think here we should have chi_file-> chi_from_x_file || chi_from_z_file, I have just added  chi_file= the same as chi_from_x_file or chi_from_z_file in the ini file for the moment*/
